...
<html>
<head>
    <title>Registrar categoriaC</title>
    <meta charset="utf-8">
...
$cp_actual = "";
    ...
    if (isset($_POST['consultar1'])) {
        ...
        {
            $consulta1= "SELECT * FROM categoria WHERE id_categoria= $ID";
            $resultados=mysqli_query($conex, $consulta1);
       

            if (mysqli_num_rows($resultados) > 0) {
                $row = mysqli_fetch_assoc($resultados);
             $id_actual = $row['id_categoria'];
...
$consulta=" UPDATE  categoria SET descripcion = '$name' WHERE id_categoria = '$ID'";
